#define VERSION "\r\nVersion of installed firmware is V1.3 - 11/2019 by jankop"
#include <Wire.h>
#include <EEPROM.h>
const uint8_t AD774X_ADDRESS = 0x48;// AD774X I2C address
//*********************** Settings! ********************************
// AD7747 default data definition for the differential input configuration
...
const uint8_t EEPROMSamplePeriod = 1;      // place for SamplePeriod variable (two bytes of EEPROM)
...
uint8_t I2C_State = 0;                     // status of I2C bus, 0 = without error, not used yet, for debugging only
...
void setup() {
  ...
  // the serial interface as described in section Serial_Terminal.
  //--------------------------------------------------------------------
  AD774X_Reset();
...
//----------------------------------------------------------------------
void loop() {
  if (EnableOffsetAutomatic)OffsetAutomatic();
