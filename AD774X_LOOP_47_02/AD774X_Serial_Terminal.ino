...
/********************** Serial Terminal Commands *********************
  ...
  fr     FLASH RESET      clears the EEPROM memory area in use, registers 07-14 are initialized from PROGMEM only
  ...
  if (Serial.available()) {
    ...
    if (SxBuff[SxBuffLength - 2] == 0x0D && SxBuff[SxBuffLength - 1] == 0x0A) {
      ...
      if ((SxBuff[SxBuffLength - 4] == 'f' || SxBuff[SxBuffLength - 4] == 'F') && (SxBuff[SxBuffLength - 3] == 'r' || SxBuff[SxBuffLength - 3] == 'R')) {
        DeleteEEPROM();
        Serial.print(F("\r\nDeleted EEPROM memory and set PROGMEM as default"));
        Serial.print(F("\r\nUse the Restart for accept this changes now"));
      }
      //----------------------------------------------------------------------
